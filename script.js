...
const inquiryTemplates = [
    ...
    { id: 'career_interview', category: 'career', title: '面試與錄取機會', subject: ['公司名稱', '應徵職位'], template: '{person}想占問，應徵「{subjectValue}」的「{subjectExtra}」職位，是否有機會順利錄取？以此起卦時間，在{location}起卦，請分析錄取機率、個人優勢、競爭與阻礙、後續時程，以及這份機會是否適合長期發展。' },
    ...
    { id: 'health_family', category: 'health', title: '家人健康關懷', subject: ['家人姓名', '與當事人關係'], template: '{person}想占問，家人「{subjectValue}」（與我的關係：{subjectExtra}）近期健康是否需要特別留意？以此起卦時間，在{location}起卦，請分析目前狀況、需注意的部位與徵兆、恢復趨勢、照護重點，以及未來三個月健康運勢。' },
    ...
    { id: 'wealth_stock', category: 'wealth', title: '是否適合買股票', subject: ['股票名稱或代號', '預計投入金額或持有資料'], placeholders: ['例如：2330／台積電／美股 NVDA', '例如：預計投入 5000／已持有 3 股，成本 912.71'], template: '{person}想占問，{stockPositionQuestion}以此起卦時間，在{location}起卦，{stockPositionAnalysis}' },
    ...
    { id: 'life_home', category: 'life', title: '地址與居住吉凶', subject: ['完整地址或社區名稱', '預計居住者或居住期間'], template: '{person}想占問，位於「{subjectValue}」的住所，是否適合「{subjectExtra}」居住？以此起卦時間，在{location}起卦，請分析此處與居住者的契合程度、居住後的整體運勢、家庭與身心影響、需要留意之處，以及長期居住是否吉利。' },
    ...
    { id: 'life_lost', category: 'life', title: '尋找遺失物品', subject: ['遺失物品', '最後看見的地點與時間'], template: '{person}想占問，遺失的「{subjectValue}」是否有機會找回？最後看見的地點與時間為「{subjectExtra}」。以此起卦時間，在{location}起卦，請分析物品可能所在方向與環境、找回機會、應優先尋找之處、可能的協助者，以及適合採取的行動。' },
...
const inquiryExampleInputs = {
    love_feeling: ['例如：張瑞塵', '例如：1999/08/09'],
    love_future: ['例如：張瑞塵', '例如：1999/08/09'],
    love_reconcile: ['例如：前任張瑞塵', '例如：1999/08/09'],
    love_confess: ['例如：張瑞塵', '例如：1998/03/12'],
    love_marriage: ['例如：交往對象張瑞塵', '例如：1997/11/20'],
    love_custom: ['例如：張瑞塵', '例如：1999/08/09'],
    career_change: ['例如：目前任職的日月公司', '例如：考慮應徵產品經理'],
    career_stay: ['例如：日月公司', '例如：產品經理'],
    career_interview: ['例如：遠景科技', '例如：前端工程師'],
    career_promotion: ['例如：日月公司', '例如：資深專員'],
    career_business: ['例如：餐飲業', '例如：與張瑞塵合夥'],
    career_custom: ['例如：電商產業', '例如：營運主管職缺'],
    health_overall: ['例如：腰部', '例如：疼痛兩週'],
    health_recovery: ['例如：膝蓋韌帶受傷', '例如：正在接受物理治療'],
    health_family: ['例如：張瑞塵', '例如：母親'],
    health_exam: ['例如：胃鏡檢查', '例如：市立醫院'],
    health_mind: ['例如：工作期限壓力', '例如：已持續兩個月'],
    health_custom: ['例如：睡眠狀況', '例如：近一個月經常失眠'],
    wealth_fortune: ['例如：正職薪資與接案收入', '例如：一年內存款十萬元'],
    wealth_stock: ['例如：2330／台積電／美股 NVDA', '例如：預計投入 5000／已持有 3 股，成本 912.71'],
    wealth_stock_target: ['例如：2330 2500／台積電 2500／美股 NVDA 201', '例如：20260930之前 400'],
    wealth_add: ['例如：台積電', '例如：每股成本 850 元'],
    wealth_stop: ['例如：台積電', '例如：成本 950 元，目前虧損 10%'],
    wealth_business: ['例如：餐飲業', '例如：預計投入 50 萬元'],
    wealth_home: ['例如：信義路一段 100 號', '例如：總價 1500 萬，預計年底前購買'],
    wealth_custom: ['例如：黃金投資', '例如：預計投入 10 萬元'],
    life_home: ['例如：信義路一段 100 號', '例如：本人與家人，預計居住五年'],
    life_move: ['例如：新北市板橋區', '例如：2026 年 10 月'],
    life_travel: ['例如：日本東京', '例如：2026/10/01 至 2026/10/07'],
    life_decision: ['例如：留任或接受新工作', '例如：2026/08/31'],
    life_lost: ['例如：黑色皮夾', '例如：昨晚 10 點，在客廳'],
    life_family: ['例如：張瑞塵（父親）', '例如：近期溝通容易爭執'],
    life_custom: ['例如：台北市住家', '例如：預計明年搬遷']
};
...
function buildStockPositionInquiry(value, extra) {
    const stockName = value.trim() || '此投資標的';
    const positionInput = extra.trim();
    const isHolding = /持有|已買|成本|均價|套牢|\d+(?:\.\d+)?\s*股/.test(positionInput);
    const amount = positionInput.match(/-?\d[\d,]*(?:\.\d+)?/)?.[0] || '';

    if (isHolding) {
        return {
            question: `目前持有「${stockName}」，相關持股資料為「${positionInput || '尚未填寫'}」，未來三至六個月是否適合繼續持有？`,
            analysis: '請聚焦分析持有期間的整體趨勢、續抱或減碼的適合時機、主要風險，以及這筆持股的發展是否有利。'
        };
    }

    const investment = positionInput.replace(/^(?:預計)?投入\s*/, '') || amount;
    return {
        question: `若以${investment ? `「${investment}」的資金` : '預定資金'}買入「${stockName}」，未來三至六個月是否有利？`,
        analysis: '請聚焦分析買入時機、未來三至六個月的整體趨勢、主要風險、可能結果，以及目前是否適合投入。'
    };
}
...
function updateInquiry(force = false) {
    ...
    const stockPositionInquiry = buildStockPositionInquiry(value, extra);
    ...
    let text = activeTemplate.custom
        ...
        : activeTemplate.template
            ...
            .replaceAll('{stockPositionQuestion}', stockPositionInquiry.question)
            .replaceAll('{stockPositionAnalysis}', stockPositionInquiry.analysis)
...
function selectTemplate(template) {
    ...
    const [valuePlaceholder = label, extraPlaceholder = extraLabel] = inquiryExampleInputs[template.id] || template.placeholders || [];
